...
def run_inference_ollama(model_name: str, dataset_name: str, wandb_run: wandb, run_on_test: bool = False, limit: int = None) -> tuple:
    ...
    dataset = load_from_disk(f"distillation-data/{dataset_name}/llama3.3:70b/giddy-star-218")
    prompts = dataset["prompt"]
    true_labels = dataset["completion"]
    pred_labels = []
...
def run_inference_hf(model_name: str, dataset_name: str, wandb_run: wandb, run_on_test: bool = False, limit: int = None) -> tuple:
    ...
    if "sentiment" in dataset_name:
        dataset = load_from_disk(f"distillation-data/sentiment/llama3.3:70b/giddy-star-218")
    prompts = dataset["prompt"]
    true_labels = dataset["completion"]
    pred_labels = []
